...
L putf_: {unsigned char *p=inv_str(&U(R1)); // invierte: nva. dir. real 1er char
	  printf((char*)p);             // traslada  
	  printf("\n");                 
          reinv_str(p,&U(R1));   	    // re-invierte                
	  GT(R0);                           // retorna
	}

L printint_: {unsigned char *p=inv_str(&U(PRINTINT)); // invierte: nva. dir. real 1er char
	  printf((char*)p,R1);             // traslada                 
          reinv_str(p,&U(PRINTINT));   	    // re-invierte 
	  printf("\n");                
	  GT(R0);                           // retorna
	}  

L printfloat_: {unsigned char *p=inv_str(&U(PRINTFLOAT)); // invierte: nva. dir. real 1er char
	  printf((char*)p,RR1);             // traslada                 
          reinv_str(p,&U(PRINTFLOAT));   	    // re-invierte
	  printf("\n");                 
	  GT(R0);                           // retorna
	}                                                           
